...
# install.packages(c("tidyverse", "furrr", "randtoolbox", "prodlim",
#                    "matlab", "tmvmixnorm", "MCMCpack", "Amelia"))
library(parallel)
n.cores = detectCores()
library(tidyverse)
library(furrr)
        ...
        for (level in 1:length(blocks)) {
          ...
          result_level <- fun_map_level(
            ...
            ~{
              ...
              # impute missing values
              if (length(code_lower) > 1 & code_higher == "336") { ####
                if (method == "MBEMMI") {
                  res <- MBEMMI(data_raw = data, 
                                n_imp = 1,
                                sobol_seq = sobol_block)[[1]]
                  ...
                  res <- EMB(data, n_imputation = 1, polytime = 3)[[1]]
                  if (length(res) == 1) {
                    res
                  }else{
                    data$detail
...
cat(paste0("Data set: ", 0, ", Method: MBEMMI ........ Start...\n"))
set.seed(1234)
result_MBEMMI = PSI(data_qcew, method = "MBEMMI", n_sim = 10)
saveRDS(result_MBEMMI, paste0("./data/imputations_MBEMMI_", 0, ".Rds"))
cat(paste0("Data set: ", 0, ", Method: MBEMMI ........ Done.\n"))

cat(paste0("Data set: ", 0, ", Method: BMMI ........ Start...\n"))
set.seed(1234)
result_BMMI = PSI(data_qcew, method = "BMMI", n_sim = 10)
saveRDS(result_BMMI, paste0("./data/imputations_BMMI_", 0, ".Rds"))
cat(paste0("Data set: ", 0, ", Method: BMMI ........ Done.\n"))

blocks <- partition_data(data_qcew)
data = blocks$`Digit-3`$`336`
res_emb <- EMB(data, n_imputation = 10, polytime = 3)

# cat(paste0("Data set: ", 0, ", Method: EMB ........ Start...\n"))
# set.seed(12)
# result_EMB = PSI(data_qcew, method = "EMB", n_sim = 10)
# saveRDS(result_EMB, paste0("./data/imputations_EMB_", 0, ".Rds"))
# cat(paste0("Data set: ", 0, ", Method: EMB ........ Done.\n"))

mean_3361_MBEMMI = result_MBEMMI %>% 
  future_map(
    ~{
      res = .
      res %>% filter(industry_code == "3361") %>% 
  ...
  future_map(
    ~{
      ...
      if (length(res) > 1) {
        ...
        data.frame(var = rep(NA_real_,20))
      }
    }
  ) %>% 
  do.call("cbind", .) %>% 
...
compare_3361 = data.frame(
  true = data_qcew$true$qtrly %>% filter(industry_code == "3361") %>% 
    select(-industry_code, -industry_title)  %>% 
    as.numeric(),
  MBEMMI = mean_3361_MBEMMI$.,
  BMMI = mean_3361_BMMI$.,
  EMB = mean_3361_EMB$.
)

compare_3361$time = 1:20

compare_3361 %>% 
  ggplot() +
  geom_line(aes(x = time, y = true), color = "red") +
  geom_line(aes(x = time, y = MBEMMI), color = "green") +
  geom_line(aes(x = time, y = BMMI), color = "blue") +
  geom_line(aes(x = time, y = EMB), color = "orange")

compare_3361 %>%
  pivot_longer(cols = -time, names_to = "Method", values_to = "Value") %>%
  ggplot(aes(x = time, y = Value, color = Method)) +
  geom_line() +
  scale_color_manual(values = c("true" = "red", "MBEMMI" = "green", "BMMI" = "blue", "EMB" = "orange")) +
  labs(color = "Method") +
  theme_minimal()
