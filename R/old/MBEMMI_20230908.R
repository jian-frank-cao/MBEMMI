...
E_step_old = function(data){
  ...
  for (type in 1:nrow(data$miss_types)) {
    ...
    if (data$miss_types[type,] %>% as.integer %>% sum != 0) {
      ...
      # sweep
      theta = sweep(theta, 1)
      theta = sweep(theta, sweep_pos)              ### try aux vars one by one
      ...
      add_var = add_var + (t(matrix(data$miss_orig[ind,],nrow = length(ind))) %*%
                             matrix(data$miss_orig[ind,],nrow = length(ind))) * theta
    }
  }
...
MSU_step_old = function(data){
  ...
  for (year in 1:n_year) {
    if (sum(data$miss_z[,year]) != 0) {
      ...
      order_m = data$miss_orig[(n_time_per_year*(year-1)+
                                  1):(n_time_per_year*year), ]*1
